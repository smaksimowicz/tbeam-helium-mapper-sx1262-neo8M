...
boolean in_deadzone = true;
boolean auto_deadzone = true;
...
bool packetQueued;
bool isJoined = false;
uint32_t gps_volts = 3200;

bool screen_stay_on = true;
...
void axp192Init() {
  if (axp192_found) {
    ...
    axp.setLDO3Voltage(gps_volts);                          // Voltage for GPS Power.  (Neo-6 , Neo-8 can take 2.7v to 3.6v)
...
void menu_experiment(void) {
  ...
  axp.setLDO3Voltage(gps_volts);                          // Voltage for GPS Power.  (Neo-6 can take 2.7v to 3.6v) 
}
...
dr_t sf_list[] = {DR_SF7, DR_SF8, DR_SF9, DR_SF10 , DR_SF11, DR_SF12};
...
struct menu_entry menu[] = {
    ...
    {"GPS Reset", menu_gps_reset},         {"GPS +100mv", menu_experiment}};
